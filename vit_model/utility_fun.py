...
def Binarization(mask, thres=0.0, type=0):
    if type == 0:
        mask = np.where(mask > thres, 1.0, 0.0)
    elif type == 1:
        mask = np.where(mask > thres, mask, 0.0)
    return mask
...
def binImage(heatmap, thres=0):
    _, heatmap_bin = cv2.threshold(
        heatmap, thres, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
    )
    # t in the paper
